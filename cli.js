...
/** Display USDC account balance */
async function printETHBalance({ address, name }) {
  console.log(`${name} USDC balance is`, web3.utils.fromWei(await web3.eth.getBalance(address)))
}
...
async function deposit({ currency, amount }) {
  ...
  await printERC20Balance({ address: smashnado._address, name: 'Smashcash', tokenAddress: config.deployments[`netId${netId}`][currency].tokenAddress })
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  if (currency === 'USDC' && refund !== '0') {
    throw new Error('The USDC purchase is supposted to be 0 for USDC withdrawals')
  }
  refund = toWei(refund)
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 137) {
        console.log(`Transaction submitted through the relay. View transaction on etherscan https://polygonscan.com/tx/${relay.data.txHash}`)
      } else {
    ...
    await smashnado.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
      .on('transactionHash', function (txHash) {
        if (netId === 137) {
          console.log(`View transaction on https://polygonscan.com/tx/${txHash}`)
        } else {
 ...
 */
async function init({ rpc, noteNetId, currency = 'usdc', amount = '1' }) {
  ...
  if (inBrowser) {
    ...
    try {
      if (web3 == undefined) {
        ...
        contractJson = await (await fetch('build/contracts/ERC20Smashnado.json')).json()
  ...
  if (!(netId === 137)) {
    throw new Error('You did not connect to Polygon network. Please make sure you connected to Polygon Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to Polygon Mainnet.')
  erc20 = currency == 'usdc' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  const instance = { currency: 'usdc', amount: '1' }
    //await init(instance)
  ...
  window.deposit = async (_currencyStr, _amountStr) => {
    ...
    let senderBalance = web3.utils.fromWei(await erc20.methods.balanceOf(senderAccount).call(), 'mwei')
    if ( parseFloat(senderBalance) < parseFloat(_amountStr) ) {
      w_display_notification('Insufficient USDC Balance in your wallet to deposit.', 'error')
      return false;
    }
    ...
    try {
      ...
      if (noteString) {
        ...
        //var resultStr = 'SmashCash Matic balance is ' + web3.utils.fromWei(await web3.eth.getBalance(smashnado._address)) +'<br/>'
